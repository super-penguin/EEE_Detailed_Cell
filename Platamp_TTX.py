...
path_to_json = 'Data_02_01/Final2/TTX'
...
#
# # ##### Plotting
df = pd.read_csv('Data_02_01/Final2/N/total_results.csv', index_col = 0)
df = df.sort_values(by = ['NMDA_weight'])
df_TTX = pd.read_csv('Data_02_01/Final2/TTX/TTX_results.csv', index_col = 0)
...
ax.scatter(df['NMDA_weight'], df['platamp'], s = 120, marker = 'o',
c = tableau(0), edgecolors = tableau(0), linewidth = 2, alpha = 0.5, label = "Normal")
ax.scatter(df_TTX['Glu_weight'], df_TTX['platamp'], s = 120, marker = '^',
c = tableau(6), edgecolors = tableau(6), linewidth = 2,alpha = 0.5, label = "TTX")
ax.set_xlim([0, 0.8])
ax.set_xlabel("Glutamate Stimulation Weight", size = 18)
...
#plt.show()
plt.savefig(path_to_json + "/GluStim_Platamp_compare.png")
# #
# # # #########################################################################
# plt.clf()
# figure = plt.figure(figsize = (9,6))
