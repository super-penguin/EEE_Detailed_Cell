...
Run 3 different condition: TTX, 3-AP and control on basal34
For generating trace comparison
...
def bAP(Bnum = 34, TTX = False, Atype = False, vec = []):
    ...
    dist = h.distance(0.73, sec = Cell.basal[Bnum])

    v_vec_dend = h.Vector()
    v_vec_dend.record(Cell.basal[Bnum](0.73)._ref_v)
    ###########################################
    ### Run & Plot
...
#    pdb.set_trace()   #Debugging
    ...
    data['recording']['dend']['voltage'] = list(v_vec_dend)
...
if __name__ == "__main__":
    ...
    V = bAP(Bnum = 34, TTX = False, Atype = False)
    bAP(Bnum = 34, TTX = True, Atype = False, vec = V)
    bAP(Bnum = 34, TTX = False, Atype = True)
