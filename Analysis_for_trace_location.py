...
path_to_json = 'Data_02_05/Fig2_no_ex/TTX/'
...
for index, js in enumerate(json_files):
    with open(os.path.join(path_to_json, js)) as json_file:
        ...
        soma_trace.append(data['recording']['soma']['voltage'][4000:])
        dend1_trace.append(data['recording']['basal_34']['voltage_0.8'][4000:])
        dend2_trace.append(data['recording']['basal_34']['voltage_0.5'][4000:])
        dend3_trace.append(data['recording']['basal_34']['voltage_0.3'][4000:])
        labels.append(data['ExNMDA']['weight'])
plt.close()
plt.clf()
...
#plt.get_major_ticks(False)
#ax.spines['top'].set_visible(False)
ax.set_frame_on(False)

Condition = "Fig2_no_ex_TTX_"
for i in range(nfile):
    #new_soma_trace[i] = soma_trace[i] + 20*i
    plt.plot(new_time, soma_trace[i], color = tableau(i), label = 'Input w = ' + str(labels[i]), linewidth = 2)
...
for i in range(nfile):
    ...
    plt.plot(new_time, dend1_trace[i],color = tableau(i), label = 'Input w = ' + str(labels[i]), linewidth = 2)
    plt.legend(loc = 'best', fontsize = 15)
...
title2 = Condition + "Basal[34]_0.8"
...
for i in range(nfile):
    ...
    plt.plot(new_time, dend2_trace[i],color = tableau(i), label = 'Input w = ' + str(labels[i]), linewidth = 2)
    plt.legend(loc = 'best', fontsize = 15)
...
title3 = Condition + "Basal[34]_0.5"
...
for i in range(nfile):
    #new_time = [x+20*i for x in time]
    plt.plot(new_time, dend3_trace[i],color = tableau(i), label = 'Input w = ' + str(labels[i]), linewidth = 2)
    plt.legend(loc = 'best', fontsize = 15)
...
title4 = Condition +"Basal[34]_0.3"
